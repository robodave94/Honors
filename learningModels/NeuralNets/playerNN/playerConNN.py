...
x = tf.placeholder(tf.float32, [None, width*height],name='x_input')
# similarly, we have a placeholder for true outputs (obtained from labels)
y_ = tf.placeholder(tf.float32, [None, nClass],name='y_output')
...
if simpleModel:
  ...
  x_image = tf.reshape(x, [-1, width, height, 1],name='x_im_input')
  ...
  # similarly for third layer, with nFeatures2 features per 5x5 patch
  # input is nFeatures2 (number of features output from previous layer)
  ''' W_conv3 = weight_variable([5, 5, nFeatures2, nFeatures2])

  h_conv3 = tf.nn.relu(conv2d(h_pool2, W_conv3) + b_conv2)
  h_pool3 = tf.nn.max_pool(h_conv3, ksize=[1, 1, 1, 1],
  ...
  h_pool3_flat = tf.reshape(h_pool2, [-1, (width / 4) * (height / 4) * nFeatures2])
  ...
  y = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2,name='y_pred')

  # measure of error of our model
  # this needs to be minimised by adjusting W and b
  #need to modify for updated model
  ...
  # finalise
  saver.save(sess,'trainedModel/playermodel',global_step =i)
  coord.request_stop()
  coord.join(threads)
